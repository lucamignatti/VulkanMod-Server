...
public class Vulkan {
    ...
    public static final Set<String> REQUIRED_EXTENSION =
    ...
    /**
     * Initialize Vulkan for headless server-side rendering (no window/surface)
     */
    public static void initVulkanHeadless() {
        createInstance();
        setupDebugMessenger();
        // Skip surface creation for headless mode
    ...
    private static void createInstance() {
        if (ENABLE_VALIDATION_LAYERS && !checkValidationLayerSupport()) {
            throw new RuntimeException(
                "Validation requested but not supported"
            );
        }
        ...
        try (MemoryStack stack = stackPush()) {
            ...
            if (ENABLE_VALIDATION_LAYERS) {
                createInfo.ppEnabledLayerNames(
                    asPointerBuffer(VALIDATION_LAYERS)
                );
    ...
    public static void setDebugLabel(
        MemoryStack stack,
        int objectType,
        long handle,
        String label
    ...
    private static void createVma() {
        try (MemoryStack stack = stackPush()) {
            VmaVulkanFunctions vulkanFunctions = VmaVulkanFunctions.calloc(
                stack
            );
